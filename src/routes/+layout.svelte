<script lang="ts">
	...
	let drawerHidden: boolean = true; // Default to hidden

	// Restore drawer state from localStorage on mount
	onMount(() => {
		const savedDrawerState = localStorage.getItem('drawerHidden');
		if (savedDrawerState !== null) {
			drawerHidden = savedDrawerState === 'true';
		}
		handleResize();
	});
	...
	const toggleDrawer = () => {
		...
		localStorage.setItem('drawerHidden', String(drawerHidden));
	};
	...
	$: console.log(data.pages); // This will show the pages data and their icons in the console
</script>
...
<Drawer
	...
	width="w-64"
	class="overflow-y-auto overflow-x-hidden pb-32"
	id="sidebar"
>
	<div class="flex items-center">
		<CloseButton on:click={() => (drawerHidden = true)} class="mb-4 dark:text-white lg:hidden" />
	</div>
	<Sidebar {activeUrl} {activeClass} {nonActiveClass} class="sbColor" asideClass="w-54">
		<SidebarWrapper divClass="overflow-y-auto py-4 px-3 rounded bg-gray-100 dark:bg-neutral-900">
			<SidebarGroup>
				<SidebarBrand {site} />
			</SidebarGroup>
			<SidebarGroup border>
				...
				<SidebarItem label="SOPs" href="/sops">
					<svelte:fragment slot="icon">
						<Icon.BookOpenSolid
							class="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
							size="30"
